...
disp.cam_pos(120,120)

player.clamp_x(120,904)
player.clamp_y(120,904)
    
def load(animal,sprite,chance,speed):
    if (random.random() * 100) < ((chance * 3 + CurrentPhase * 5) ** 0.031) and len(enemies)<CurrentPhase*2:
        ...
        enemies.append([gameObj(sprite, int(screenx), int(screeny)),animal,50 if animal=="bear" else 20,int(speed),int(tempx),int(tempy)])
...
def poad(animal,sprite,chance):
    if (random.random() * 100) < ((chance * 3 + CurrentPhase * 8) ** 0.031) and len(food)<CurrentPhase*4:
        ...
        food.append([gameObj(sprite, int(screenx), int(screeny)),animal,2,int(tempx),int(tempy)])
...
def Update(dt):
    ...
    #RedFox spawn rate
    load("redfox",e_redfox,4,2)

    #PolarBear spawn rate
    load("bear",e_bear,1,1)
                
    #Owl spawn rate
    load("owl",e_owl,2,2)

    #Eagle spawn rate
    load("eagle",e_eagle,2,3)
    ...
    for enemy in enemies:
        ...
        if RandomVariable % 10 == 0:
            if abs(enemy[-2]-disp.cam_pos_x()-120) > 120 or abs(enemy[-1]-disp.cam_pos_y()-120) > 120:
                enemies.pop(count)
                gc.collect()
            RandomVariable=0
        if enemy[0].collider(player):
            print(enemy)
            print(enemy[0].pos_x, enemy[0].pos_y, player.pos_x, player.pos_y)
        count+=1
        ...
        if enemy[1] == "redfox":
            enemy[0].sprite(e_redfox, 360)
        elif enemy[1] == "owl":
            enemy[0].sprite(e_owl, 360)
        elif enemy[1] == "eagle":
            enemy[0].sprite(e_eagle, 360)
        elif enemy[1] == "bear":
            enemy[0].sprite(e_bear, 360)
    ...
    for foo in food:
        ...
        if RandomVariable % 10 == 0:
            if abs(foo[-2]-disp.cam_pos_x()) > 200 or abs(foo[-1]-disp.cam_pos_y()) > 200:
                food.pop(count)
                gc.collect()
            RandomVariable=0
        count+=1
...
def Draw(dt):
    ...
    xmap.draw(disp.cam_pos_x()-120, disp.cam_pos_y()-120)
    ...
    for enemy in enemies:
        ...
        #draw.text(str(enemy[-2]) +", "+ str(enemy[-1]),enemy[0].pos_x,enemy[0].pos_y-10,8)
        #draw.text(str(enemy[0].pos_x) + "," + str(enemy[0].pos_y), enemy[0].pos_x, enemy[0].pos_y+10,9)
    for foo in food:
        foo[0].draw()
